...
pub mod reserved {
    ...
    pub const Q_EMPTY_SIG: Signal = Signal::reserved(0);

    /// Entry action signal — perform one-time setup when entering the state.
    pub const Q_ENTRY_SIG: Signal = Signal::reserved(1);
    ...
    pub const Q_EXIT_SIG: Signal = Signal::reserved(2);
    ...
    pub const Q_INIT_SIG: Signal = Signal::reserved(3);
    ...
    pub const Q_USER_SIG: Signal = Signal::user(4);
}
