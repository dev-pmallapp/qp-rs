...
/// Register the RF Active Object to receive interrupt event posts.
pub fn register_rf_ao(ao: &'static dyn ActiveRunnable) {
    RF_AO.store(ao as *const _ as *mut _, Ordering::Release);
}

fn read_sx_irq_status() -> (PhyEvent, hal::rf::RxMetadata) {
    // Scaffolding: SPI transaction inside NVIC interrupt.
    (PhyEvent::TxDone, hal::rf::RxMetadata::default())
}

/// Generic EXTI handler for external transceivers on Cortex-M.
#[no_mangle]
...
/// Configure the NVIC priority settings for EXTI line.
pub fn configure_rf_interrupt() {
    // Scaffolding: set priority of EXTI4_15 numerically greater than QK_BASEPRI
}
